...
Route::middleware(['participant'])->group(function () {
    ...
    Route::post(
        '/wi/video/{video}/events/{event}/attempt',
...
Route::get('/results/{participant}', [ResultAdminController::class, 'show'])->name('results.show');
        ...
        Route::prefix('wi/{wi}')
            ...
            ->group(function () {
                ...
                Route::get('/videos/{video}/events', [WiVideoEventAdminController::class, 'index'])
                    ->name('wi.videos.events.index');

                Route::post('/videos/{video}/events', [WiVideoEventAdminController::class, 'store'])
                    ->name('wi.videos.events.store');

                Route::put('/videos/{video}/events/{event}', [WiVideoEventAdminController::class, 'update'])
                    ->name('wi.videos.events.update');

                Route::delete('/videos/{video}/events/{event}', [WiVideoEventAdminController::class, 'destroy'])
